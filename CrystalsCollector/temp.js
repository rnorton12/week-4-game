...
var targetNum = 0; // the number the user has to match to win
var totalScore = 0; // the users total score which needs to match the target number to win
var wins = 0;
var losses = 0;
var buttonClicks = 0;

// contains the properties for each crystal
var crystalObjects = {
    crystalOne: {
        id: "button1",
...
// an array containing properties for each crystal
crystalItems = [
...
$(document).ready(function () {
    ...
    // To generate random numbers from a given range:
    ...
    // generate and return a random value for the crystal object
    function generateCrystalValue() {
        return (getRandomNumber(crystalNumRange[0], crystalNumRange[1]));
    }

    // update the value attribute for each crystal object
    ...
    // generate the web page content for the crystal objects
    function generateCrystals() {
        for (var i = 0; i < crystalItems.length; i++) {
            // Create a new button element
            var crystalBtn = $("<button>"); //Equivalent: $(document.createElement('button'))
            crystalBtn.attr("id", crystalItems[i].id); // set the button id
...
　
            ...
            image.attr("id", crystalItems[i].image.id); // set the image id
    ...
    // update the total score, wins, and losses
    ...
    // these functions will execute first time page is loaded
    generateNumberToGuess();
    generateCrystals();
    updateStatistics();
